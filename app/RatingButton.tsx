...
const RatingButton: React.FC<RatingButtonProps> = ({ id, time, handleClick, children, position, solutionToggled }) => {
  ...
  return (
    <button 
        id={id}
        className={`flex flex-col flex-grow justify-center items-center px-2 py-3 md:my-0 md:mb-4 ${canRate ? 'bg-gray-300 text-gray-400 hover:bg-gray-300': 'bg-indigo-600 hover:bg-indigo-400 text-white'} font-bold rounded-md`}
        onClick={handleClick}
    >
