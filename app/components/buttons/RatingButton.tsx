...
const RatingButton: React.FC<RatingButtonProps> = ({ id, time, handleClick, children, position, solutionToggled, sidePadding }) => {
  ...
  return (
    <button 
        id={id}
        className={`flex flex-col flex-grow justify-center items-center ${sidePadding} py-3 md:my-0 md:mb-4 bg-slate-700 text-slate-300 ${canRate ? 'opacity-25' : 'active:bg-slate-500 hover:bg-slate-600'} font-bold rounded-md`}
        onClick={handleClick}
    >
